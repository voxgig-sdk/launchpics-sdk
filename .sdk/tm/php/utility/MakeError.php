...
class LaunchpicsMakeError
{
    public static function call(?LaunchpicsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new LaunchpicsContext([], null);
        }
        $op = $ctx->op ?? new LaunchpicsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new LaunchpicsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof LaunchpicsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new LaunchpicsError('', $msg, $ctx);
        ...
        if ($err instanceof LaunchpicsError) {
