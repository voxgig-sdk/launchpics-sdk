...
module LaunchpicsUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = LaunchpicsContext.new({}, nil)
    end
    op = ctx.op || LaunchpicsOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || LaunchpicsResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(LaunchpicsError) ? err.msg : err.to_s
    ...
    sdk_err = LaunchpicsError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(LaunchpicsError)
