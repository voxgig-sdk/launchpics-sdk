...
module LaunchpicsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LaunchpicsResponse.new({})
    ...
    if fetch_err
      ...
      response = LaunchpicsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LaunchpicsResponse.new(fetched)
    else
