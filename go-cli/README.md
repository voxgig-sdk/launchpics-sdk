...
boru-driven command-line client **and** interactive REPL for the Launchpics
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/launchpics-cli)
make build
...
LAUNCHPICS_BASE=https://api.example.com ./launchpics-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/launchpics-cli
   ```
...
Run with no arguments to open a REPL (prompt `launchpics>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ai_processing`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/launchpics-cli`. |
...
(`./launchpics-cli <expr>`) and interactively (the REPL), and expressions compose the
