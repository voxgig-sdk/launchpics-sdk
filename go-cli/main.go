// Command Launchpics-cli is a CLI + REPL for the Launchpics SDK,
...
const prompt = "launchpics"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "ai_processing health image"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LAUNCHPICS_APIKEY (api key), LAUNCHPICS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LAUNCHPICS_APIKEY carries the API key and
	// LAUNCHPICS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LAUNCHPICS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LAUNCHPICS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLaunchpicsSDK(opts)
