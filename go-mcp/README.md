...
[MCP](https://modelcontextprotocol.io) server exposing the Launchpics SDK as
two agent tools — `launchpics_list` and `launchpics_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/launchpics-mcp)
make build

# 2. Provide credentials via the environment
export LAUNCHPICS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user launchpics \
  -- /absolute/path/to/launchpics-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./launchpics-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// launchpics_list: first page of records
{ "entity": "ai_processing" }
{ "entity": "ai_processing", "query": { } }

// launchpics_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/launchpics-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user launchpics \
     -- "$PWD"/dist/*/launchpics-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `launchpics_list` and `launchpics_load` tools now appear
   in new sessions. Ask the agent to *"list ai_processing using launchpics"*
   and it calls `launchpics_list` with `{"entity":"ai_processing"}`.
...
```sh
export LAUNCHPICS_APIKEY=sk_live_xxx            # API key
export LAUNCHPICS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `launchpics_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `launchpics_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LAUNCHPICS_APIKEY` | API key sent with every request. |
| `LAUNCHPICS_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

ai_processing | health | image

### Smoke test via HTTP (raw JSON-RPC)

```sh
./launchpics-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"launchpics_load","arguments":{"entity":"health","query":{"id":1}}}}'
```
