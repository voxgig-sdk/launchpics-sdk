...
type Args struct {
	Entity string         `json:"entity" jsonschema:"ai_processing | health | image"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.LaunchpicsSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "launchpics_list",
		Description: "List records from Launchpics. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "launchpics_load",
		Description: "Load a single record from Launchpics. " +
...
func runOp(client *sdk.LaunchpicsSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.LaunchpicsSDK, name string) (sdk.LaunchpicsEntity, error) {
	switch strings.ToLower(name) {
	case "ai_processing":
		return client.AiProcessing(nil), nil
	case "health":
