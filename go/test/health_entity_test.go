...
func TestHealthEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LAUNCHPICS_TEST_HEALTH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func healthBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LAUNCHPICS_TEST_HEALTH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LAUNCHPICS_TEST_HEALTH_ENTID"])
