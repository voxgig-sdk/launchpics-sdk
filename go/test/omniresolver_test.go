...
import (
	...
	omni "github.com/voxgig-sdk/launchpics-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LaunchpicsError); is {
				out := map[string]any{
					"name":    "LaunchpicsError",
...
func omniCtx(arg any, client *sdk.LaunchpicsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LaunchpicsSDK); is {
			client = live
