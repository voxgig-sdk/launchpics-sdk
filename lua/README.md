...
The Lua SDK for the Launchpics API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:AiProcessing()` — each with the same small set of operations (`load`, `create`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/launchpics-sdk/releases)),
...
Create an instance: `local ai_processing = client:AiProcessing(nil)`
...
```
lua/
├── launchpics_sdk.lua    -- Main SDK module
...
The main module (`launchpics_sdk`) exports the SDK constructor
