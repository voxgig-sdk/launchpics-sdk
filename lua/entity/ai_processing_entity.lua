...
local AiProcessingEntity = {}
AiProcessingEntity.__index = AiProcessingEntity


function AiProcessingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiProcessingEntity)
  self._name = "ai_processing"
  self._client = client
...
function AiProcessingEntity:get_name()
  return self._name
end


function AiProcessingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiProcessingEntity.new(self._client, opts)
end
...
function AiProcessingEntity:mark_deleted()
  self._deleted = true
end


function AiProcessingEntity:deleted()
  return true == self._deleted
end


function AiProcessingEntity:data_set(args)
  if args ~= nil then
...
function AiProcessingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiProcessingEntity:match_set(args)
  if args ~= nil then
...
function AiProcessingEntity:match_get()
...
function AiProcessingEntity:stream(action, args, callopts)
...
---@param reqdata AiProcessingCreateData
---@param ctrl? table
---@return AiProcessing
---@return string? err
function AiProcessingEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function AiProcessingEntity:_run_op(ctx, post_done)
  local utility = self._utility
