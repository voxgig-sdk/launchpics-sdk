...
// Launchpics SDK AiProcessing entity
...
class AiProcessingEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AiProcessingEntity($this->_client, $opts);
    }

    /**
     * @param AiProcessing|array $args AiProcessing data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = LaunchpicsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return AiProcessing|array The current AiProcessing data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of AiProcessing fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = LaunchpicsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of AiProcessing fields).
     */
    ...
    /**
     * Create a new AiProcessing.
     *
     * @param AiProcessingCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed AiProcessingCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return AiProcessing|array The created AiProcessing as an assoc-array at the
     *   SDK boundary; throws LaunchpicsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = LaunchpicsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
