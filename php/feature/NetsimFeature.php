...
class LaunchpicsNetsimFeature extends LaunchpicsBaseFeature
{
    ...
    private function _simulate(LaunchpicsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(LaunchpicsContext $ctx, array $applied): void
    {
        $client = $this->client;
