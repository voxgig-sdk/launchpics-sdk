...
class LaunchpicsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = LaunchpicsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = LaunchpicsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!LaunchpicsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, LaunchpicsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return LaunchpicsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = LaunchpicsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = LaunchpicsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = LaunchpicsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new LaunchpicsError($op . "_allow",
                "LaunchpicsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = LaunchpicsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new LaunchpicsError("graphql_error",
                "LaunchpicsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_ai_processing = null;

    // Canonical facade: $client->AiProcessing()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->ai_processing()
    // resolves here too.
    public function AiProcessing($data = null)
    {
        require_once __DIR__ . '/entity/ai_processing_entity.php';
        if ($data === null) {
            if ($this->_ai_processing === null) {
                $this->_ai_processing = new AiProcessingEntity($this, null);
            }
            return $this->_ai_processing;
        }
        return new AiProcessingEntity($this, $data);
    }
