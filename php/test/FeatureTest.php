...
class FtCtrl extends LaunchpicsControl
{
...
class FtClient
{
    ...
    private LaunchpicsUtility $utility;

    public function __construct(LaunchpicsUtility $utility, array $options)
    {
    ...
    public function get_utility(): LaunchpicsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LaunchpicsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LaunchpicsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LaunchpicsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LaunchpicsConfig::make_feature($name);
    ...
    private static function buildUrl(LaunchpicsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LaunchpicsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LaunchpicsResponse($fetched);
        }
    ...
    private function fail(LaunchpicsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LaunchpicsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
