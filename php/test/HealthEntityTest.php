...
class HealthEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LAUNCHPICS_TEST_HEALTH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function health_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LAUNCHPICS_TEST_HEALTH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["LAUNCHPICS_TEST_HEALTH_ENTID"]);
