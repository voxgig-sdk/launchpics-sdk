...
from launchpics_sdk.utility import register

# Load features
from launchpics_sdk.feature.base_feature import LaunchpicsBaseFeature
from launchpics_sdk.features import _has_feature, _make_feature
...
class LaunchpicsSDK:
    ...
    def __init__(self, options=None):
        ...
        utility = LaunchpicsUtility()
        self._utility = utility

        from launchpics_sdk.config import shared_config
    ...
    def get_utility(self):
        return LaunchpicsUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("LaunchpicsSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def AiProcessing(self, data=None) -> "AiProcessingEntity":
        """Entity factory: client.AiProcessing().list() / client.AiProcessing().load({"id": ...})."""
        from launchpics_sdk.entity.ai_processing_entity import AiProcessingEntity
        return AiProcessingEntity(self, data)


    def Health(self, data=None) -> "HealthEntity":
        """Entity factory: client.Health().list() / client.Health().load({"id": ...})."""
        from launchpics_sdk.entity.health_entity import HealthEntity
        return HealthEntity(self, data)


    def Image(self, data=None) -> "ImageEntity":
        """Entity factory: client.Image().list() / client.Image().load({"id": ...})."""
        from launchpics_sdk.entity.image_entity import ImageEntity
        return ImageEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "LaunchpicsSDK":
        if sdkopts is None:
            sdkopts = {}
