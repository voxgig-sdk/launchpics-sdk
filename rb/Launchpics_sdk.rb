...
require_relative 'Launchpics_types'
...
class LaunchpicsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = LaunchpicsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = LaunchpicsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, LaunchpicsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    LaunchpicsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = LaunchpicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = LaunchpicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = LaunchpicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = LaunchpicsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = LaunchpicsError.new(
        "graphql_error", "LaunchpicsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.AiProcessing.list / client.AiProcessing.load({ "id" => ... })
  def AiProcessing(data = nil)
    require_relative 'entity/ai_processing_entity'
    AiProcessingEntity.new(self, data)
  end
