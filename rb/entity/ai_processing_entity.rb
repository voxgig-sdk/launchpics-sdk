...
class AiProcessingEntity
  ...
  def make
    opts = @_entopts.dup
    AiProcessingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LaunchpicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [AiProcessing, Hash] the current AiProcessing data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LaunchpicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of AiProcessing fields)
  def match_get
  ...
  #
  # @param reqdata [AiProcessingCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [AiProcessing, Hash] the created AiProcessing; raises LaunchpicsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = LaunchpicsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
