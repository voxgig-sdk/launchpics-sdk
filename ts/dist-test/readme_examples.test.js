...
const TEST_SEED = { "entity": { "ai_processing": { "test01": { "id": "test01" } }, "health": { "test01": { "id": "test01" } }, "image": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+LaunchpicsSDK\b/.test(code) ||
        /\bLaunchpicsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LaunchpicsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LaunchpicsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
